...
if [[ "$PROFILE" == "web" || "$PROFILE" == "WEB" ]];then
  mvn clean install -U help:active-profiles -Ppayara-ci-managed,web -f target/security-tck-4.0.0/tck/pom.xml
else
  mvn clean install -U help:active-profiles -Ppayara-ci-managed -f target/security-tck-4.0.0/tck/pom.xml
fi
